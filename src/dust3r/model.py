...
def load_model(
    ...
    verbose=True,
    gaussian_head=False,
    gaussian_feature_dim=64,
    gaussian_scale_min=1e-4,
    gaussian_scale_init=1e-2,
    gaussian_scale_max=3e-2,
    gaussian_opacity_init=0.1,
    gaussian_2d_head=False,
    gaussian_2d_feature_dim=64,
    gaussian_2d_scale_min=1e-4,
    gaussian_2d_scale_init=1e-2,
    gaussian_2d_scale_max=5e-2,
    gaussian_2d_opacity_init=0.5,
):
    if verbose:
    ...
    if gaussian_head and "gaussian_head=" not in args:
        args = args.replace(
            "ARCroco3DStereoConfig(",
            (
                "ARCroco3DStereoConfig("
                f"gaussian_head=True, gaussian_feature_dim={gaussian_feature_dim}, "
                f"gaussian_scale_min={gaussian_scale_min}, "
                f"gaussian_scale_init={gaussian_scale_init}, "
                f"gaussian_scale_max={gaussian_scale_max}, "
                f"gaussian_opacity_init={gaussian_opacity_init}, "
            ),
            1,
        )
    if gaussian_2d_head and "gaussian_2d_head=" not in args:
        args = args.replace(
            "ARCroco3DStereoConfig(",
            (
                "ARCroco3DStereoConfig("
                f"gaussian_2d_head=True, "
                f"gaussian_2d_feature_dim={gaussian_2d_feature_dim}, "
                f"gaussian_2d_scale_min={gaussian_2d_scale_min}, "
                f"gaussian_2d_scale_init={gaussian_2d_scale_init}, "
                f"gaussian_2d_scale_max={gaussian_2d_scale_max}, "
                f"gaussian_2d_opacity_init={gaussian_2d_opacity_init}, "
            ),
            1,
        )
...
class ARCroco3DStereoConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **croco_kwargs,
        ...
        self.gaussian_opacity_init = gaussian_opacity_init
        self.gaussian_2d_head = gaussian_2d_head
        self.gaussian_2d_feature_dim = gaussian_2d_feature_dim
        self.gaussian_2d_scale_min = gaussian_2d_scale_min
        self.gaussian_2d_scale_init = gaussian_2d_scale_init
        self.gaussian_2d_scale_max = gaussian_2d_scale_max
        self.gaussian_2d_opacity_init = gaussian_2d_opacity_init
...
class ARCroco3DStereo(CroCoNet):
    ...
    def __init__(self, config: ARCroco3DStereoConfig):
        ...
        self.gaussian_feature_dim = config.gaussian_feature_dim
        self.gaussian_scale_min = config.gaussian_scale_min
        self.gaussian_scale_init = config.gaussian_scale_init
        self.gaussian_scale_max = config.gaussian_scale_max
        self.gaussian_opacity_init = config.gaussian_opacity_init
        self.gaussian_2d_feature_dim = config.gaussian_2d_feature_dim
        self.gaussian_2d_scale_min = config.gaussian_2d_scale_min
        self.gaussian_2d_scale_init = config.gaussian_2d_scale_init
        self.gaussian_2d_scale_max = config.gaussian_2d_scale_max
        self.gaussian_2d_opacity_init = config.gaussian_2d_opacity_init
        ...
        self.set_downstream_head(
            ...
            config.pose_head,
            config.gaussian_head,
            config.gaussian_2d_head,
            **self.croco_args,
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kw):
        if os.path.isfile(pretrained_model_name_or_path):
            return load_model(
                ...
                device="cpu",
                gaussian_head=kw.pop("gaussian_head", False),
                gaussian_feature_dim=kw.pop("gaussian_feature_dim", 64),
                gaussian_scale_min=kw.pop("gaussian_scale_min", 1e-4),
                gaussian_scale_init=kw.pop("gaussian_scale_init", 1e-2),
                gaussian_scale_max=kw.pop("gaussian_scale_max", 3e-2),
                gaussian_opacity_init=kw.pop("gaussian_opacity_init", 0.1),
                gaussian_2d_head=kw.pop("gaussian_2d_head", False),
                gaussian_2d_feature_dim=kw.pop(
                    "gaussian_2d_feature_dim", 64
                ),
                gaussian_2d_scale_min=kw.pop(
                    "gaussian_2d_scale_min", 1e-4
                ),
                gaussian_2d_scale_init=kw.pop(
                    "gaussian_2d_scale_init", 1e-2
                ),
                gaussian_2d_scale_max=kw.pop(
                    "gaussian_2d_scale_max", 5e-2
                ),
                gaussian_2d_opacity_init=kw.pop(
                    "gaussian_2d_opacity_init", 0.5
                ),
                **kw,
            )
    ...
    def set_downstream_head(
        ...
        **kw,
        ...
        self.downstream_head = head_factory(
            ...
            has_gaussian=bool(gaussian_head),
            has_gaussian_2d=bool(gaussian_2d_head),
        )
    ...
    def gaussian_parameters(self):
        if not hasattr(self.downstream_head, "dpt_gaussian"):
            raise RuntimeError("Model was created without gaussian_head=True")
        return self.downstream_head.dpt_gaussian.parameters()

    def freeze_non_gaussian_parameters(self):
        for parameter in self.parameters():
            parameter.requires_grad_(False)
        for parameter in self.gaussian_parameters():
            parameter.requires_grad_(True)

    def gaussian_2d_parameters(self):
        if not hasattr(self.downstream_head, "dpt_gaussian_2d"):
            raise RuntimeError("Model was created without gaussian_2d_head=True")
        return self.downstream_head.dpt_gaussian_2d.parameters()

    def freeze_non_gaussian_2d_parameters(self):
        for parameter in self.parameters():
            parameter.requires_grad_(False)
        for parameter in self.gaussian_2d_parameters():
            parameter.requires_grad_(True)
