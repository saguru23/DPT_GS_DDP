...
def prepare_output(outputs, outdir, revisit=1, use_pose=True):
    ...
    reset_mask_all = torch.cat([view["reset"] for view in outputs["views"]], 0).bool()
    shifted_reset_mask = torch.cat(
        [torch.zeros(1, dtype=torch.bool), reset_mask_all[:-1]], dim=0
    )
    kept_view_indices = torch.arange(len(outputs["views"]))[~shifted_reset_mask]
    has_overlap_after = torch.zeros_like(reset_mask_all)
    has_overlap_after[:-1] = shifted_reset_mask[1:]
    state_indices = kept_view_indices + 1
    reset_with_overlap = reset_mask_all[~shifted_reset_mask] & has_overlap_after[
        ~shifted_reset_mask
    ]
    state_indices[reset_with_overlap] += 1
    ...
    reset_mask = reset_mask_all[~shifted_reset_mask]
    ...
    pr_poses_local = torch.cat(pr_poses, 0)
    identity = torch.eye(4, device=pr_poses_local.device)
    local_to_global = identity.unsqueeze(0).repeat(len(pr_poses_local), 1, 1)
    raymap_local_to_global = local_to_global.clone()

    if reset_mask.any():
        reset_poses = torch.where(
            reset_mask.unsqueeze(-1).unsqueeze(-1), pr_poses_local, identity
        )
        cumulative_bases = matrix_cumprod(reset_poses)
        shifted_bases = torch.cat([identity.unsqueeze(0), cumulative_bases[:-1]], dim=0)
        local_to_global = shifted_bases
        raymap_local_to_global = shifted_bases.clone()
        raymap_local_to_global[reset_mask] = cumulative_bases[reset_mask]

    pr_poses_global = torch.einsum(
        "bij,bjk->bik", local_to_global, pr_poses_local
    )
    pr_poses = list(pr_poses_global.unsqueeze(1).unbind(0))
    ...
    cam_dict = {
        ...
        "R": R_c2w.cpu().numpy(),
        "t": t_c2w.cpu().numpy(),
        "state_idx": state_indices.cpu().numpy(),
        "local_to_global": local_to_global.cpu().numpy(),
        "raymap_local_to_global": raymap_local_to_global.cpu().numpy(),
    }
