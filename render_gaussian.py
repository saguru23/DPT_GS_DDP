...
from dust3r.gaussian_render import render_prediction_view, sample_gaussians  # noqa: E402
from dust3r.model import ARCroco3DStereo  # noqa: E402
from dust3r.utils.device import todevice  # noqa: E402


def parse_args():
    parser = argparse.ArgumentParser("Render a DPT-GS checkpoint")
...
def save_gaussian_ply(
    ...
    if ply_format == "supersplat":
        save_supersplat_ply(path, means, colors, opacities, scales, rotations)
        return

    save_pointcloud_ply(path, means, colors, opacities, scales, rotations)


def save_pointcloud_ply(path, means, colors, opacities, scales, rotations):
    ...
    with Path(path).open("w") as file:
        ...
        for xyz, color, opacity, scale, rotation in zip(
            means, rgb, opacities, scales, rotations
        ):
            file.write(
                f"{xyz[0]:.8f} {xyz[1]:.8f} {xyz[2]:.8f} "
                f"{int(color[0])} {int(color[1])} {int(color[2])} "
                f"{opacity:.8f} "
                f"{scale[0]:.8f} {scale[1]:.8f} {scale[2]:.8f} "
...
def save_supersplat_ply(path, means, colors, opacities, scales, rotations):
    ...
    # Graphdeco/3DGS PLY stores activated RGB/opacity/scale as SH DC,
    # opacity logits, and log-scales.
...
def main():
    ...
    model = ARCroco3DStereo.from_pretrained(
        args.model_path,
        gaussian_head=True,
        gaussian_feature_dim=config["gaussian_feature_dim"],
        gaussian_scale_min=config["gaussian_scale_min"],
        gaussian_scale_init=config["gaussian_scale_init"],
        gaussian_scale_max=config["gaussian_scale_max"],
        gaussian_opacity_init=config["gaussian_opacity_init"],
    )
    model.downstream_head.dpt_gaussian.load_state_dict(
    ...
    for view_index in range(min(args.num_views, len(output.ress))):
        ...
        if not args.skip_render:
            rendered, _, alpha, target = render_prediction_view(
        ...
        print(
            f"view={view_index} "
            f"scale={pred['gaussian_scales'].mean().item():.5f} "
            f"opacity={pred['gaussian_opacity'].mean().item():.4f} "
