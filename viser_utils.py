...
class PointCloudViewer:
    def __init__(
        ...
        self.frame_state_indices = np.asarray(
            cam_dict.get("state_idx", np.arange(self.num_frames) + 1)
        )
        self.raymap_local_to_global = np.asarray(
            cam_dict.get(
                "raymap_local_to_global",
                np.repeat(np.eye(4)[None], self.num_frames, axis=0),
            )
        )
        self.current_timestep = 0
        self.raymap_prediction_count = 0
        self.raymap_inference_running = False
        ...
        focal_values = np.asarray(cam_dict.get("focal", [533]), dtype=np.float32)
        finite_focals = focal_values[np.isfinite(focal_values)]
        initial_focal = float(np.median(finite_focals)) if len(finite_focals) else 533
    ...
    @staticmethod
    def generate_intrinsics(h, w, focal):
        return np.array(
            [[focal, 0, w / 2.0], [0, focal, h / 2.0], [0, 0, 1]],
            dtype=np.float32,
        )

    def get_frame_resolution(self, frame_idx):
        step = self.all_steps[frame_idx]
        pc = np.asarray(self.pcs[step]["pc"])
        if pc.ndim < 3:
            raise ValueError(f"Cannot infer raymap resolution from point cloud shape {pc.shape}.")
        h, w = int(pc.shape[-3]), int(pc.shape[-2])
        return h, w
    ...
    def _connect_client(self, client: viser.ClientHandle):
        ...
        fov_panel = client.gui.add_text(
            "fov:", f"{2 * np.arctan((self.size / 2.0)/self.focal_slider.value) * 180 / np.pi}"
        )
        aspect_panel = client.gui.add_text("aspect:", f"{client.camera.aspect}")
        ...
        def _(_: viser.CameraHandle):
            with self.server.atomic():
                ...
                fov_panel.value = (
                    f"{2 * np.arctan((self.size / 2.0)/self.focal_slider.value) * 180 / np.pi}"
                )
                aspect_panel.value = f"{client.camera.aspect}"
        ...
        def _(_) -> None:
            try:
                if not self.state_args:
                    print(
                        "Cannot infer raymap: recurrent model state is empty. "
                        "Run initial image inference before setting the current camera."
                    )
                    return
                if self.raymap_inference_running:
                    print("Raymap inference is already running. Please wait.")
                    return

                self.raymap_inference_running = True
                gui_set_current_camera.disabled = True

                cam = self.get_camera_state(client)
                global_pose = cam.c2w
                frame_idx = min(self.current_timestep, self.num_frames - 1)
                h, w = self.get_frame_resolution(frame_idx)
                focal = float(self.focal_slider.value)
                intrins = self.generate_intrinsics(h, w, focal)
                cam.fov = 2 * np.arctan((w / 2.0) / focal)
                cam.aspect = w / h
                local_to_global = self.raymap_local_to_global[frame_idx]
                local_pose = np.linalg.inv(local_to_global) @ global_pose
                raymap = torch.from_numpy(self.get_ray_map(local_pose, h, w, intrins))[
                    None
                ].float()
                
                
                state_idx = min(
                    int(self.frame_state_indices[frame_idx]),
                    len(self.state_args) - 1,
                )
                print(
                    f"Using recurrent state {state_idx}/{len(self.state_args) - 1} "
                    f"and chunk-local {h}x{w} rays for playback frame {frame_idx}."
                )
                view = {
                    "img": torch.full((1, 3, h, w), torch.nan),
                    "ray_map": raymap,
                    "true_shape": torch.from_numpy(np.int32([raymap.shape[1:-1]])),
                    "idx": self.current_timestep,
                    "instance": f"raymap_{self.raymap_prediction_count}",
                ...
                output = inference_step(
                    view, self.state_args[state_idx], self.model, device=self.device
                )
                print("Finish Inference Raymap")
                pts3ds = output["pred"]["pts3d_in_self_view"].cpu().numpy()
                pts3ds = geotrf(global_pose[None], pts3ds)
                colors = 0.5 * (output["pred"]["rgb"].cpu().numpy() + 1.0)
                conf = output["pred"]["conf"].cpu().numpy()
                pts3ds, colors = self.parse_pc_data(
                    pts3ds, colors, conf, set_border_color=True,
                    downsample_factor=self.downsample_slider.value
                )
                prediction_name = f"/raymap_predictions/{self.raymap_prediction_count}"
                self.raymap_prediction_count += 1
                self.pc_handles.append(
                    self.server.add_point_cloud(
                        name=f"{prediction_name}/pred_pts",
                        points=pts3ds,
                        colors=colors,
                        point_size=0.005,
                    )
                ...
                self.raymap_inference_running = False
                gui_set_current_camera.disabled = False
    ...
    def animate(self):
        ...
        prev_timestep = int(gui_timestep.value)
